...
export default function PublicFeed() {
  ...
  return (
    <main className="min-h-screen bg-beige-retro text-brown-dark p-4 md:p-6 lg:p-12 font-mono animate-fade-in-up">
      <div className="max-w-5xl mx-auto overflow-x-hidden md:overflow-visible">
        
        {/* Navigation Header */}
        <header className="border-b-4 border-brown-dark pb-6 mb-8 md:mb-12 flex flex-col md:flex-row justify-between items-start md:items-end gap-6 md:gap-0">
          <div className="flex flex-col gap-2 w-full md:w-auto">
            <h1 className="text-2xl md:text-3xl font-black tracking-tighter uppercase break-all">Trading_Journal</h1>
          ...
          <div className="flex border-2 border-brown-dark overflow-hidden w-full md:w-fit shadow-[4px_4px_0px_0px_rgba(74,55,33,1)]">
            <button 
              onClick={() => { setView("list"); setSelectedDate(null); }}
              className={`flex-1 md:flex-none px-4 py-2 text-xs font-black cursor-pointer uppercase transition-all duration-150 ${
                view === "list" 
            ...
            <button 
              ...
              className={`flex-1 md:flex-none px-4 py-2 text-xs font-black cursor-pointer uppercase border-l-2 border-brown-dark transition-all duration-150 ${
                view === "calendar" 
                  ? "bg-brown-dark text-brown-medium shadow-[inset_3px_3px_0px_0px_rgba(30,20,10,0.5)]" 
                  : "bg-brown-medium text-brown-dark hover:bg-beige-muted"
              }`}
        ...
        {view === "calendar" && (
          <div className="bg-beige-muted border-2 border-brown-dark p-4 md:p-8 shadow-[4px_4px_0px_0px_rgba(74,55,33,1)] md:shadow-[8px_8px_0px_0px_rgba(74,55,33,1)] mb-12 animate-fade-in-up w-full">
            <div className="flex justify-between items-center mb-8">
              ...
              <h2 className="text-base md:text-xl font-black uppercase tracking-widest transition-all">
            ...
            {/* Scrollable container specifically for the calendar grid */}
        ...
        <div key={`feed-${view}-${selectedDate || 'all'}`} className="space-y-10 md:space-y-16">
          {filteredLogs.length === 0 ? (
            <div className="border-4 border-dashed border-brown-light p-10 md:p-20 text-center bg-beige-muted/50 animate-fade-in-up">
            ...
            filteredLogs.map((log, index) => {
              ...
              return (
                <article 
                  key={log.id} 
                  className="border-2 border-brown-dark bg-beige-muted shadow-[4px_4px_0px_0px_rgba(74,55,33,1)] md:shadow-[8px_8px_0px_0px_rgba(74,55,33,1)] animate-in transition-all duration-300 hover:-translate-y-1 hover:shadow-[6px_6px_0px_0px_rgba(74,55,33,1)] md:hover:shadow-[12px_12px_0px_0px_rgba(74,55,33,1)]"
                ...
                >
                  ...
                  >
                    <div className="flex flex-col md:flex-row md:items-center gap-2 md:gap-4">
                      <span className="text-lg md:text-2xl font-black italic tracking-widest uppercase">
                        LOG_{log.date?.toDate().toLocaleDateString('en-GB')}
                      </span>
                      <span className="w-fit text-[10px] border border-beige-retro px-2 py-1 uppercase font-black opacity-70">
                        {log.daily_bias}
                      </span>
                    </div>
                  ...
                  <div className={`log-content-wrapper ${isExpanded ? 'expanded' : ''}`}>
                    <div className="log-content-inner">
                      <div className="p-4 bg-brown-light/10 border-b border-brown-dark text-[10px] font-black text-brown-dark uppercase flex flex-col md:flex-row gap-2 md:gap-4">
                        <span className="text-red-800 tracking-tighter underline shrink-0">HIGH_IMPACT_NEWS:</span>
                        <span>{log.news_events || "NONE_DETECTED"}</span>
                      </div>

                      <div className="p-4 md:p-6 space-y-10 md:space-y-12">
                        {log.trades?.map((trade: any, idx: number) => (
                          <div key={idx} className="border-l-4 border-brown-dark pl-4 md:pl-6 space-y-4">
                            <div className="flex justify-between items-end border-b border-brown-light pb-2">
                              <h3 className="text-xl md:text-3xl font-black uppercase tracking-tighter">{trade.pair}</h3>
                              <span className={`text-base md:text-lg font-black ${trade.pl >= 0 ? "text-green-800" : "text-red-800"}`}>
