      ...
      <div className="max-w-5xl mx-auto space-y-12">
        ...
        <header className="border-b-4 border-brown-dark pb-6 flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
          ...
          <div className="flex gap-2">
            <button 
              onClick={() => router.push("/")}
              className="text-[10px] font-black bg-brown-dark text-beige-retro px-4 py-2 uppercase border-2 border-brown-dark hover:bg-brown-medium cursor-pointer transition-colors"
            >
              [ VIEW_FEED ]
            </button>
            {isAdmin && (
              <button 
                ...
                className="text-[10px] font-black bg-beige-retro text-brown-dark px-4 py-2 uppercase border-2 border-brown-dark hover:bg-brown-dark hover:text-beige-retro cursor-pointer transition-colors"
              >
                [ SETTINGS ]
              </button>
            )}
            <button 
              ...
              className="text-[10px] font-black bg-red-800 text-beige-retro px-4 py-2 uppercase border-2 border-brown-dark hover:bg-red-900 cursor-pointer transition-colors"
            >
              [ LOGOUT ]
            </button>
          </div>
        ...
        {/* Range Selector */}
        <div className="flex border-2 border-brown-dark overflow-hidden bg-brown-medium w-fit shadow-[4px_4px_0px_0px_rgba(74,55,33,1)]">
          {(["7D", "30D", "YTD", "ALL"] as Range[]).map((r, i) => (
            <button 
              key={r}
              onClick={() => setRange(r)}
              className={`px-6 py-2 text-xs font-black cursor-pointer uppercase ${i !== 0 ? 'border-l-2 border-brown-dark' : ''} ${range === r ? "bg-brown-dark text-brown-medium" : "hover:bg-beige-muted text-brown-dark bg-beige-retro"}`}
            >
              {r}
            </button>
          ))}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          ...
          {/* Today's Profit */}
          <div className={`border-2 border-brown-dark p-6 shadow-[8px_8px_0px_0px_rgba(74,55,33,1)] ${stats.todayPl >= 0 ? (stats.todayPl === 0 ? "bg-beige-muted" : "bg-green-100/40") : "bg-red-100/40"}`}>
          ...
          <div className="border-2 border-brown-dark p-6 bg-beige-muted shadow-[8px_8px_0px_0px_rgba(74,55,33,1)]">
          ...
          {/* Avg Win vs Avg Loss */}
          <div className="border-2 border-brown-dark p-6 bg-beige-muted shadow-[8px_8px_0px_0px_rgba(74,55,33,1)] flex flex-col justify-center">
